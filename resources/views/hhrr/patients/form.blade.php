...
@section('content')
    <a href="{{ $patient->exists ? route('hhrr.patients.show', $patient) : route('clinical.psr.admissions.index') }}" class="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-700 mb-3">
        <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i> {{ $patient->exists ? 'Back to patient' : 'Back to admissions' }}
    </a>
    ...
    <form method="POST" action="{{ $patient->exists ? route('hhrr.patients.update', $patient) : route('hhrr.patients.store') }}"
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ $patient->exists ? route('hhrr.patients.show', $patient) : route('clinical.psr.admissions.index') }}" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900">Cancel</a>

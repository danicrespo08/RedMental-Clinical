...
<style>
    ...
    .narr-block { padding:.85rem 1rem; border-left:3px solid #fda4af; background:#fff1f2; border-radius:0 .5rem .5rem 0; margin-bottom:.75rem; }
    .narr-label { font-size:.6rem; font-weight:800; color:#9f1239; text-transform:uppercase; letter-spacing:.05em; }
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3.5">
                <a href="{{ route('clinical.psr.discharges.index') }}" class="w-9 h-9 rounded-lg bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-600 transition-colors border border-slate-200 flex-shrink-0">
                    <i data-lucide="arrow-left" class="w-4 h-4"></i>
                </a>
                <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-rose-400 to-rose-600 text-white flex items-center justify-center font-black text-lg shadow-md shadow-rose-500/25">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(! $discharge->is_signed)
                    @can('clinical.psr.discharges.edit')
                        <a href="{{ route('clinical.psr.discharges.edit', $discharge) }}" class="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-xs font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5">
                            <i data-lucide="pencil" class="w-3.5 h-3.5"></i> Edit
                        </a>
                    @endcan
                    @can('clinical.psr.discharges.sign')
                        <form method="POST" action="{{ route('clinical.psr.discharges.sign', $discharge) }}" class="inline" data-confirm="Sign this discharge? The admission will be marked as discharged.">@csrf
                            <button class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5">
                                <i data-lucide="pen-tool" class="w-3.5 h-3.5"></i> Sign &amp; close admission
                            </button>
                        </form>
                    @endcan
                @endif
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
        <div class="stat-card">
            ...
            <div class="stat-value text-emerald-600">{{ $discharge->total_sessions_attended }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Sessions absent</div>
            <div class="stat-value text-rose-500">{{ $discharge->total_sessions_absent }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Units billed</div>
            <div class="stat-value text-violet-600">{{ $discharge->total_units_billed }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Days in program</div>
            <div class="stat-value text-blue-600">{{ $discharge->days_in_program }}</div>
        </div>
        <div class="stat-card">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-1 space-y-4">
            <div class="dc-section">
                <div class="dc-hd"><div class="dc-num">i</div><div><div class="dc-title">Episode summary</div></div></div>
                <div class="dc-body space-y-2 text-[12px]">
            ...
            <div class="dc-section">
                <div class="dc-hd"><div class="dc-num"><i data-lucide="heart-pulse" class="w-3.5 h-3.5"></i></div><div><div class="dc-title">Diagnoses</div></div></div>
                <div class="dc-body space-y-3">
        ...
        <div class="lg:col-span-2 space-y-4">
            <div class="dc-section">
                <div class="dc-hd"><div class="dc-num">1</div><div><div class="dc-title">Clinical course</div></div></div>
                <div class="dc-body">
            ...
            <div class="dc-section">
                <div class="dc-hd"><div class="dc-num">2</div><div><div class="dc-title">Aftercare plan</div></div></div>
                <div class="dc-body">
                    @php $hasAny = false; @endphp
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                <div class="sig-box {{ $discharge->is_signed ? '' : 'draft' }}">
                    <div>
                        ...
                        <div class="text-xs text-slate-500 mt-0.5">{{ $discharge->therapist?->credentials ?? '' }}</div>
                    </div>

            ...
            <div class="space-y-2">
                @foreach($pending as $adm)
                    <div class="flex items-center justify-between gap-3 bg-white border border-amber-200 rounded-lg px-3 py-2">
                        ...
                        @can('clinical.psr.discharges.create')
                            <a href="{{ route('clinical.psr.discharges.create', ['admission_id' => $adm->id]) }}" class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[10px] font-bold uppercase tracking-wider rounded-md inline-flex items-center gap-1 whitespace-nowrap">

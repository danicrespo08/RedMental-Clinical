    ...
    <form method="POST" action="{{ $isEdit ? route('clinical.tcm.authorizations.update', $auth) : route('clinical.tcm.authorizations.store') }}">
        ...
        <div class="tcm-section">
            ...
            <div class="tcm-body grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <div><label class="field-label">Used units</label><div class="field-input field-mono" style="background:#f8fafc;color:#475569;cursor:default;">{{ $auth->used_units ?? 0 }}{{ $auth->approved_units ? ' / ' . $auth->approved_units : '' }} <span style="font-size:.6rem;color:#94a3b8;font-family:sans-serif;">tracked from service log</span></div></div>
            </div>
        </div>

@extends('layouts.app')
@section('title', 'TCM — Authorization ' . $auth->auth_number)
...
<div class="max-w-7xl mx-auto">
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                @can('clinical.tcm.authorizations.edit')
                    <a href="{{ route('clinical.tcm.authorizations.edit', $auth) }}" class="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-xs font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="stat-card"><div class="stat-label">Approved units</div><div class="stat-value text-orange-600">{{ $auth->approved_units }}</div></div>
        <div class="stat-card"><div class="stat-label">Used units</div><div class="stat-value text-blue-600">{{ $auth->used_units }}</div></div>
        <div class="stat-card"><div class="stat-label">Remaining</div><div class="stat-value text-emerald-600">{{ $remaining }}</div></div>
        <div class="stat-card">
            <div class="stat-label">Utilization</div>
            <div class="stat-value text-amber-600">{{ $pct }}%</div>
            <div class="h-1.5 bg-slate-100 rounded-full overflow-hidden mt-1">
                <div class="h-full bg-gradient-to-r from-orange-400 to-amber-600" style="width: {{ $pct }}%;"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-1 space-y-4">
            <div class="tcm-section">
                <div class="tcm-hd"><div class="tcm-num">i</div><div><div class="tcm-title">Details</div></div></div>
                <div class="tcm-body space-y-2 text-[12px]">
                    ...
                    <div class="flex justify-between"><span class="text-slate-400 font-bold">CPT codes</span>
                        <span class="font-mono font-bold text-slate-700 text-right">@if(is_array($auth->cpt_codes)){{ implode(', ', $auth->cpt_codes) }}@else—@endif</span>
                    </div>
                </div>
            </div>

            <div class="tcm-section">
                <div class="tcm-hd"><div class="tcm-num"><i data-lucide="calendar-range" class="w-3.5 h-3.5"></i></div><div><div class="tcm-title">Date ranges</div></div></div>
                <div class="tcm-body space-y-3 text-[12px]">
                    <div>
        ...
        <div class="lg:col-span-2 space-y-4">
            @if($auth->denial_reason)
                <div class="tcm-section">
                    ...
                    <div class="tcm-body text-[13px] text-slate-700 whitespace-pre-line">{{ $auth->denial_reason }}</div>
                </div>
            @endif
            @if($auth->notes)
                <div class="tcm-section">
                    ...
                    <div class="tcm-body text-[13px] text-slate-700 whitespace-pre-line">{{ $auth->notes }}</div>
                </div>
            @endif
            @if(!$auth->denial_reason && !$auth->notes)
                <div class="tcm-section"><div class="tcm-body text-center py-8 text-slate-400 italic text-sm">No notes documented.</div></div>
            @endif
        </div>
    </div>
</div>

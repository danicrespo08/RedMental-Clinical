@extends('layouts.app')
@section('title', 'TCM — Authorizations')
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-xs font-bold uppercase tracking-widest text-orange-500">TCM · Authorizations</div>
            ...
            @can('clinical.tcm.authorizations.create')
                <a href="{{ route('clinical.tcm.authorizations.create') }}" class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-2 shadow-md shadow-orange-500/25">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($auths as $auth)
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3">
                            <a href="{{ route('clinical.tcm.authorizations.show', $auth) }}" class="flex items-center gap-2.5 hover:text-orange-700">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="font-mono text-[12px] font-bold text-orange-600">{{ $auth->used_units }} / {{ $auth->approved_units }}</span>
                            @if($auth->approved_units > 0)
                                <div class="h-1.5 bg-slate-100 rounded-full overflow-hidden mt-1 max-w-[80px] mx-auto">
                                    <div class="h-full bg-gradient-to-r from-orange-400 to-amber-600" style="width: {{ min(100, ($auth->used_units / $auth->approved_units) * 100) }}%;"></div>
                                </div>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-center">
                        ...
                        <td class="px-4 py-3 text-right">
                            @include('hhrr._shared._action_buttons', [
                                'showRoute'   => route('clinical.tcm.authorizations.show', $auth),
                                'editRoute'   => auth()->user()->can('clinical.tcm.authorizations.edit') ? route('clinical.tcm.authorizations.edit', $auth) : null,
                                'deleteRoute' => auth()->user()->can('clinical.tcm.authorizations.delete') ? route('clinical.tcm.authorizations.destroy', $auth) : null,

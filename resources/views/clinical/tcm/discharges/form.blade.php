...
<div class="max-w-5xl mx-auto">
    ...
    <form method="POST" action="{{ $isEdit ? route('clinical.tcm.discharges.update', $discharge) : route('clinical.tcm.discharges.store') }}">
        @csrf
        ...
        <div class="tcm-section">
            <div class="tcm-hd"><div class="tcm-num">1</div><div><div class="tcm-title">Discharge details</div></div></div>
            <div class="tcm-body grid grid-cols-2 md:grid-cols-3 gap-3">
                <div><label class="field-label">Discharge date *</label><input type="date" name="discharge_date" required value="{{ old('discharge_date', optional($discharge->discharge_date)->format('Y-m-d') ?? now()->toDateString()) }}" class="field-input"></div>
                <div>
                ...
                <div>
                    ...
                    <select name="case_manager_id" class="field-select">
                        <option value="">—</option>
                        @foreach($caseManagers as $cm)<option value="{{ $cm->id }}" @selected(old('case_manager_id', $discharge->case_manager_id) == $cm->id)>{{ $cm->full_name }}</option>@endforeach
                    </select>
                </div>
                <div>
        ...
        <div class="tcm-section">
            <div class="tcm-hd"><div class="tcm-num">2</div><div><div class="tcm-title">Diagnoses (ICD-10)</div></div></div>
        ...
        <div class="tcm-section">
            <div class="tcm-hd"><div class="tcm-num">3</div><div><div class="tcm-title">Case-management course</div><div class="tcm-sub">Coordination summary, response, barriers, risk</div></div></div>
        ...
        <div class="tcm-section">
            <div class="tcm-hd"><div class="tcm-num">4</div><div><div class="tcm-title">Aftercare &amp; community resources</div></div></div>
            <div class="tcm-body grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="field-label">Aftercare level</label>
                    <input type="text" name="aftercare_level" class="field-input" maxlength="60" value="{{ old('aftercare_level', $discharge->aftercare_level) }}" placeholder="e.g. Self-management with primary care">
                </div>
                @foreach([
        ...
        <div class="tcm-section">
            <div class="tcm-hd"><div class="tcm-num">5</div><div><div class="tcm-title">Episode metrics</div></div></div>
            <div class="tcm-body grid grid-cols-2 md:grid-cols-3 gap-3">
                <div><label class="field-label">Total contacts</label><input type="number" name="total_contacts" min="0" value="{{ old('total_contacts', $discharge->total_contacts ?? 0) }}" class="field-input field-mono"></div>
        ...
        <div class="flex items-center justify-between gap-3 pt-2 pb-6">
            ...
            <div class="flex items-center gap-2">
                ...
                <button class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5"><i data-lucide="save" class="w-4 h-4"></i> {{ $isEdit ? 'Save changes' : 'Save draft' }}</button>
            </div>
        </div>
    </form>
</div>

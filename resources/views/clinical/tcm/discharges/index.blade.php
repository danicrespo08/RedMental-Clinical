...
<div class="max-w-7xl mx-auto">
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="text-xs font-bold uppercase tracking-widest text-orange-500">TCM · Discharge summaries</div>
                <h1 class="text-xl font-black text-slate-800">Closed admissions</h1>
                <p class="text-[11px] text-slate-400 font-semibold mt-0.5">End-of-episode wrap-up · aftercare plan · prognosis</p>
            </div>
        </div>
    </div>
                ...
                @foreach($pending as $adm)
                    <div class="flex items-center justify-between gap-3 bg-white border border-amber-200 rounded-lg px-3 py-2">
                        ...
                        @can('clinical.tcm.discharges.create')
                            <a href="{{ route('clinical.tcm.discharges.create', ['admission_id' => $adm->id]) }}" class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[10px] font-bold uppercase tracking-wider rounded-md inline-flex items-center gap-1 whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($discharges as $d)
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-center"><span class="font-mono font-bold text-[12px]">{{ $d->total_contacts }}</span></td>
                        <td class="px-4 py-3 text-center"><span class="font-mono font-bold text-[12px] text-slate-700">{{ $d->days_in_program }}</span></td>
                        ...
                        <td class="px-4 py-3 text-right">
                            @include('hhrr._shared._action_buttons', [
                                'showRoute'   => route('clinical.tcm.discharges.show', $d),
                                'editRoute'   => $d->is_signed ? null : (auth()->user()->can('clinical.tcm.discharges.edit') ? route('clinical.tcm.discharges.edit', $d) : null),
                                'deleteRoute' => $d->is_signed ? null : (auth()->user()->can('clinical.tcm.discharges.delete') ? route('clinical.tcm.discharges.destroy', $d) : null),
                                'deleteLabel' => 'this discharge',
                            ])
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="8" class="px-5 py-16 text-center text-slate-400 text-sm">
                        ...
                        No discharges yet — start one from a patient's TCM admission.
                    </td></tr>
                @endforelse
            </tbody>
        </table>

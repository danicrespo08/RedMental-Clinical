...
<div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h1 class="text-xl font-black text-slate-800">Billable case-management encounters</h1>
                    <p class="text-[11px] text-slate-400 font-semibold mt-0.5">CPT-level entries fed into the weekly superbill</p>
                </div>
            </div>
            @can('clinical.tcm.service_log.create')
                <a href="{{ route('clinical.tcm.service_log.create') }}" class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-2 shadow-md shadow-orange-500/25">
    ...
    <form method="GET" class="bg-white border border-slate-200 rounded-2xl p-3 mb-4 flex items-end gap-3 flex-wrap shadow-sm">
        ...
        @if($status || $month)
            <a href="{{ route('clinical.tcm.service_log.index') }}" class="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 text-[11px] font-bold uppercase rounded-lg border border-slate-200">Clear</a>
        @endif
    </form>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($logs as $log)
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3"><div class="font-semibold text-slate-700 text-[12px]">{{ $log->service_date->format('M j, Y') }}</div></td>
                        <td class="px-4 py-3">
                            @if($patient)
                                <a href="{{ route('clinical.tcm.service_log.show', $log) }}" class="flex items-center gap-2.5 hover:text-orange-700">
                                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-orange-400 to-amber-600 text-white flex items-center justify-center font-black text-[10px]">
                        ...
                        <td class="px-4 py-3 text-[12px] text-slate-600">{{ $log->caseManager?->full_name ?? '—' }}</td>
                        <td class="px-4 py-3 text-center"><span class="font-mono text-[10px] font-bold bg-orange-50 text-orange-700 border border-orange-200 px-1.5 py-0.5 rounded">{{ $log->cpt_code }}{{ $log->modifier ? ' '.$log->modifier : '' }}</span></td>
                        ...
                        <td class="px-4 py-3 text-right">
                            @include('hhrr._shared._action_buttons', [
                                'showRoute'   => route('clinical.tcm.service_log.show', $log),
                                'editRoute'   => auth()->user()->can('clinical.tcm.service_log.edit') ? route('clinical.tcm.service_log.edit', $log) : null,
                                'deleteRoute' => auth()->user()->can('clinical.tcm.service_log.delete') ? route('clinical.tcm.service_log.destroy', $log) : null,

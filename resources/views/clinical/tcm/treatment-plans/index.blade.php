@extends('layouts.app')
@section('title', 'TCM — Service plans')
                    ...
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-right">
                            @include('hhrr._shared._action_buttons', [
                                'showRoute'   => route('clinical.tcm.treatment_plans.show', $plan),
                                'editRoute'   => $plan->is_signed ? null : (auth()->user()->can('clinical.tcm.treatment_plans.edit') ? route('clinical.tcm.treatment_plans.edit', $plan) : null),
                                'deleteRoute' => $plan->is_signed ? null : (auth()->user()->can('clinical.tcm.treatment_plans.delete') ? route('clinical.tcm.treatment_plans.destroy', $plan) : null),
                                'deleteLabel' => 'this service plan',
                            ])
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-5 py-12 text-center text-slate-400 text-sm">No service plans yet — create one from a patient's TCM admission.</td></tr>
                @endforelse
            </tbody>
        </table>

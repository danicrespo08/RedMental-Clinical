...
<style>
    ...
    .goal-code-badge { background:linear-gradient(135deg,#ea580c,#f97316); color:#fff; padding:3px 12px; border-radius:20px; font-weight:700; font-size:.72rem; }
    .objective-card { background:#fff7ed; border-left:3px solid #fb923c; padding:.75rem 1rem; margin:.5rem 0 .5rem 1.25rem; border-radius:0 .5rem .5rem 0; }
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div class="flex items-center gap-3.5">
                <a href="{{ route('clinical.tcm.treatment_plans.index') }}" class="w-9 h-9 rounded-lg bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-orange-600 transition-colors border border-slate-200 flex-shrink-0">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    ...
                    @can('clinical.tcm.treatment_plans.edit')<a href="{{ route('clinical.tcm.treatment_plans.edit', $plan) }}" class="px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5"><i data-lucide="pencil" class="w-3.5 h-3.5"></i> Edit</a>@endcan
                    @can('clinical.tcm.treatment_plans.sign')
                        <form method="POST" action="{{ route('clinical.tcm.treatment_plans.sign', $plan) }}" class="inline">@csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-1 space-y-4">
            <div class="tcm-section">
                <div class="tcm-hd"><div class="tcm-num">i</div><div><div class="tcm-title">Plan period</div></div></div>
                <div class="tcm-body space-y-2 text-[12px]">
                    ...
                    <div class="flex justify-between"><span class="text-slate-400 font-bold">Goals</span><span class="font-bold text-orange-600">{{ $goalsCount }}</span></div>
                    <div class="flex justify-between"><span class="text-slate-400 font-bold">Objectives</span><span class="font-bold text-orange-600">{{ $objectivesCount }}</span></div>
                </div>
            </div>

            <div class="tcm-section">
                <div class="tcm-hd"><div class="tcm-num"><i data-lucide="user-check" class="w-3.5 h-3.5"></i></div><div><div class="tcm-title">Case manager</div></div></div>
                <div class="tcm-body text-[13px] font-bold text-slate-700">{{ $admission?->caseManager?->full_name ?? '—' }}</div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
            @foreach([
                ...
                'coordination_strategy'=> ['Coordination strategy', '4'],
            ] as $f => $meta)
                <div class="tcm-section">
                    <div class="tcm-hd"><div class="tcm-num">{{ $meta[1] }}</div><div><div class="tcm-title">{{ $meta[0] }}</div></div></div>
                    <div class="tcm-body">
                        @if($plan->{$f})
            ...
            <div class="tcm-section">
                <div class="tcm-hd"><div class="tcm-num">5</div><div><div class="tcm-title">Goals &amp; objectives ({{ $goalsCount }})</div></div></div>
                <div class="tcm-body">
                            ...
                            @foreach($goal->objectives as $obj)
                                <div class="objective-card">
                                    <div class="flex items-start justify-between">
                                        <div>
                                            <span class="font-bold text-orange-600 text-[.78rem]">{{ $obj->objective_code }}</span>

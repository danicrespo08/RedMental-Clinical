@extends('layouts.app')
@section('title', 'IT — Master Treatment Plan')
...
<style>
    ...
    .field-input:focus, .field-textarea:focus { outline:none; border-color:#7c3aed; box-shadow:0 0 0 3px rgba(124,58,237,.08); }
    ...
    .goal-card { background:linear-gradient(180deg,#f5f3ff 0%,#ffffff 100%); border:1.5px solid #c7d2fe; border-radius:.85rem; padding:14px; margin-bottom:14px; position:relative; }
    .goal-card::before { content:''; position:absolute; top:0; left:0; bottom:0; width:4px; background:linear-gradient(180deg,#4338ca,#7c3aed); border-radius:.85rem 0 0 .85rem; }
    .goal-pill { display:inline-block; padding:4px 10px; border-radius:99px; background:linear-gradient(135deg,#4338ca,#7c3aed); color:#fff; font-size:.65rem; font-weight:800; text-transform:uppercase; letter-spacing:.04em; }
    .obj-card { background:#fff; border:1px solid #e2e8f0; border-left:3px solid #818cf8; border-radius:.55rem; padding:10px 12px; margin-bottom:6px; }
    .obj-pill { display:inline-block; padding:2px 8px; border-radius:99px; background:#eef2ff; color:#4338ca; font-size:.6rem; font-weight:800; }
    .ai-btn {
...
<div class="max-w-5xl mx-auto"
     x-data="{
        ...
        async aiSuggestGoals() {
            ...
            this.aiBusy = true;
            try {
                const res = await fetch('{{ route('clinical.it.treatment_plans.ai_suggest_goals', $admission) }}', {
                    method: 'POST',
                    headers: {
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
                ...
                <h1 class="text-xl font-black text-slate-800 truncate">{{ $plan->exists ? 'Edit treatment plan' : 'New treatment plan' }}</h1>
    ...
    <form method="POST" action="{{ $plan->exists ? route('clinical.it.treatment_plans.update', $plan) : route('clinical.it.treatment_plans.store') }}">
        @csrf
        ...
        <div class="it-section">
            <div class="it-hd"><div class="it-num">1</div><div><div class="it-title">Plan period</div></div></div>
            <div class="it-body grid grid-cols-2 gap-3">
                <div>
        ...
        {{-- 2. Narrative --}}
        <div class="it-section">
            <div class="it-hd"><div class="it-num">2</div><div><div class="it-title">Clinical narrative</div></div></div>
            <div class="it-body grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="field-label">Presenting problem</label>
                    <textarea name="presenting_problem" {{ $isSigned ? 'disabled' : '' }} class="field-textarea" placeholder="Why is the patient presenting for individual therapy?">{{ old('presenting_problem', $plan->presenting_problem) }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="discharge_criteria" {{ $isSigned ? 'disabled' : '' }} class="field-textarea" placeholder="What conditions indicate readiness for discharge?">{{ old('discharge_criteria', $plan->discharge_criteria) }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="field-label">Interventions</label>
                    <textarea name="interventions" {{ $isSigned ? 'disabled' : '' }} class="field-textarea" placeholder="Therapeutic modalities planned: CBT, DBT, EMDR, motivational interviewing, exposure, mindfulness…">{{ old('interventions', $plan->interventions) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="it-section">
            <div class="it-hd">
                <div class="it-num">3</div>
                <div class="flex-1"><div class="it-title">Goals &amp; objectives</div><div class="it-sub">Measurable, time-bound</div></div>
                @unless($isSigned)
                    <span class="flex items-center gap-2">
                        ...
                        <button type="button" class="px-3 py-1.5 bg-violet-50 text-violet-700 border border-violet-200 text-[10px] font-bold uppercase tracking-wider rounded-md inline-flex items-center gap-1"
            ...
            <div class="it-body">
                <template x-for="(g, gi) in goals" :key="gi">
                    <div class="goal-card">
                        ...
                        {{-- Objectives --}}
                        <div class="mt-3 pt-3 border-t border-dashed border-violet-200">
                            <div class="flex justify-between items-center mb-2">
                                <span class="obj-pill">Objectives (<span x-text="g.objectives.length"></span>)</span>
                                @unless($isSigned)
                                    <button type="button" class="px-2 py-1 bg-violet-50 text-violet-700 border border-violet-200 text-[10px] font-bold uppercase rounded-md inline-flex items-center gap-1"
                            ...
                            <template x-for="(o, oi) in g.objectives" :key="oi">
                                <div class="obj-card">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <input type="text" :name="`goals[${gi}][objectives][${oi}][intervention_type]`" x-model="o.intervention_type" {{ $isSigned ? 'disabled' : '' }} placeholder="Intervention (CBT, DBT…)" class="input-clean text-[11px]">
                ...
                <template x-if="goals.length === 0">
                    <div class="border-2 border-dashed border-violet-200 rounded-xl p-6 text-center text-violet-500">
    ...
    @if($plan->exists && ! $isSigned)
        @can('clinical.it.treatment_plans.sign')
            <form method="POST" action="{{ route('clinical.it.treatment_plans.sign', $plan) }}" class="text-right pb-6" data-confirm="Sign this treatment plan? It will be locked.">@csrf

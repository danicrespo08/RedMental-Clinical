...
<div class="max-w-7xl mx-auto">
    <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-4 shadow-sm">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-1.5 mt-1">
                        ...
                        <span class="text-slate-200">|</span>
                        <span class="font-mono text-[10px] font-bold bg-violet-50 text-violet-700 border border-violet-200 px-1.5 py-0.5 rounded">{{ $log->cpt_code }}{{ $log->modifier ? ' '.$log->modifier : '' }}</span>
                    </div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                ...
                @can('clinical.it.service_log.edit')
                    <a href="{{ route('clinical.it.service_log.edit', $log) }}" class="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-xs font-bold uppercase tracking-wider rounded-lg inline-flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="stat-card">
            ...
            <div class="stat-value text-violet-600">{{ $log->units }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Paid amount</div>
            <div class="stat-value text-emerald-600">{{ $log->paid_amount ? '$'.number_format((float) $log->paid_amount, 2) : '—' }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Note attached</div>
            <div class="stat-value {{ $log->has_progress_note ? 'text-emerald-600' : 'text-slate-300' }}">
                @if($log->has_progress_note)<i data-lucide="check" class="w-6 h-6 inline"></i>@else—@endif
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Diagnosis</div>
            <div class="stat-value text-rose-600 text-base">{{ $log->diagnosis_code ?: '—' }}</div>
        </div>
    </div>
        ...
        <div class="lg:col-span-1 space-y-4">
            <div class="it-section">
                ...
                <div class="it-body space-y-2 text-[12px]">
                    ...
                    <div class="flex justify-between"><span class="text-slate-400 font-bold">Therapist</span><span class="font-semibold text-slate-700 text-right">{{ $log->therapist?->full_name ?? '—' }}</span></div>
            ...
            <div class="it-section">
                <div class="it-hd"><div class="it-num"><i data-lucide="receipt" class="w-3.5 h-3.5"></i></div><div><div class="it-title">Billing</div></div></div>
                <div class="it-body space-y-2 text-[12px]">
        ...
        <div class="lg:col-span-2 space-y-4">
            @if($log->denial_reason)
                <div class="it-section">
                    ...
                    <div class="it-body text-[13px] text-slate-700 whitespace-pre-line">{{ $log->denial_reason }}</div>
                </div>
            @endif
            @if($log->notes)
                <div class="it-section">
                    ...
                    <div class="it-body text-[13px] text-slate-700 whitespace-pre-line">{{ $log->notes }}</div>
                </div>
            @endif
            @if(! $log->denial_reason && ! $log->notes)
                <div class="it-section"><div class="it-body text-center py-8 text-slate-400 italic text-sm">No additional notes documented.</div></div>
            @endif
        </div>
    </div>
</div>
